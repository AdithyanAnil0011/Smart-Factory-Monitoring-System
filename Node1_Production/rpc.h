...
void rpc_init(PubSubClient* mqtt, char* buf, uint8_t bufLen);  //initialise the rpc system

/* This is the function to pass to mqttClient.setCallback() */
void rpc_mqttCallback(char* topic, byte* payload, unsigned int length); 
 //whenever the thingsboard sends a command, PubSubClient automatically calls this func
